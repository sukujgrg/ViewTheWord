...
- Settings is native AppKit in `AppKit/SettingsWindowController.swift`: `NSTabViewController` toolbar panes, draft-backed `NSSlider` controls with aligned point readouts, and an `NSTableView` library. Slider tracking saves a whole-point `Double` on mouse-up; keyboard/accessibility actions save immediately. File import, replacement, notices, and removal confirmations belong to the Settings window; closing it cancels its attached sheet and declines unresolved replacements. `scripts/render-navigation-review.sh --settings-only --onscreen` checks appearance fixtures, slider events/persistence (including exact one-point arrows and Option-arrows at range limits), library states, and sheet lifecycle. `--inspect-settings` selects standalone manual inspection; closing its Settings window ends the run. Translation picker headings now have more label/control and table separation. The native Settings panes, imported-row selection, removal cancellation, file-panel cancellation, and accessibility value persistence were checked with computer use in an isolated fixture on 2026-09-11; continuous physical dragging and VoiceOver remain manual checks.
...
- Empty catalogs return no `BibleSources`, never a fake path or `/dev/null`. Show import instructions, disable translation pickers, and prevent navigation/search/projection from creating readers. Stop live output owned by an open tab with a shared explanation; importing restores browsing and clears the empty-catalog notice without automatically reopening stopped output. The live controller observes catalog availability even after all passage tabs close; typed notices prevent recovery from dismissing unrelated projection failures. With frozen output and an empty catalog, Space has no enabled selection; Stop and Escape remain available.
...
- Native tab headings show the browsed reference and translation abbreviations, with full names in tooltips. Only the source tab gets a leading green dot followed by native `Live` text, or a muted `● Blanked` marker. The green dot is an 8-point, non-template `NSImage` in an `NSTextAttachment` inside `NSWindowTab.attributedTitle`; AppKit dims title foreground colors in unselected tabs but preserves full-color image attachments. Draw with `NSColor.systemGreen` at the requested scale and appearance. Keep `Live`/`Blanked` as text in the attributed title, plain title, and tooltip for accessibility. Native selected-tab styling stays intact. The shared footer continues to describe the actual projected verse independently of browsing.
- Search headings, tooltips, and selection callbacks use the same `visibleSearchPage` as the table. Ignore selection from results whose sources or catalog revision are outdated, including input before the queued render. Keep `searchRequest` after a failure for translation-change retries; it does not imply visible search results.
...
- Settings/import flows: `ViewTheWord/AppKit/SettingsWindowController.swift`
