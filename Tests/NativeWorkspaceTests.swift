...
final class NativeWorkspaceTests: XCTestCase {
    ...
    func testTranslationPickersAndTabHeadingsFollowOnlyTheirOwnPassageAndLiveOwnership() async throws {
        ...
        XCTAssertNil(first.window?.tab.accessoryView)
        XCTAssertEqual(second.window?.tab.title, "● Live · Psalm 23:2 · ALT")
        XCTAssertEqual(second.window?.tab.attributedTitle?.string, "\u{FFFC} Live · Psalm 23:2 · ALT")
        let attachment = try XCTUnwrap(second.window?.tab.attributedTitle?.attribute(.attachment, at: 0, effectiveRange: nil) as? NSTextAttachment)
        XCTAssertEqual(attachment.image?.isTemplate, false)
        XCTAssertNil(second.window?.tab.accessoryView)
        a.toggleBlank(nil)
        b.browse("Genesis")
        try await settle(b)
        XCTAssertEqual(second.window?.tab.title, "● Blanked · Genesis · ALT")
        XCTAssertEqual(second.window?.tab.attributedTitle?.string, "● Blanked · Genesis · ALT")
        ...
        XCTAssertNil(second.window?.tab.attributedTitle?.attribute(.attachment, at: 0, effectiveRange: nil))
