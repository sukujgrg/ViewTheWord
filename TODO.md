...
- [ ] **Cache `BibleUrl.getAvailableBibleUrls()` results** — `Db.swift`
...
- [ ] **`BookmarkStore.contains()` — O(n) linear scan called per visible row per render** — `AppConstants.swift:384`
...
- [ ] **`HistoryStore.groupedSections` recomputed on every access** — `AppConstants.swift:142`
...
- [ ] **`NSRegularExpression` compiled on every verse query parse** — `RxVerse.swift:288`
...
- [ ] **`onChange` with computed array values causes per-render allocation** — `ContentView.swift:381,446`
...
- [ ] **No SQLite prepared statement caching for hot-path queries** — `Db.swift`
...
- [ ] **Redundant `rebuildChapterRows` on `showOnlyPrimary` toggle** — `ContentView.swift:1065` + `VerseRowView.swift:354`
...
- [ ] **Linear verse lookup in `MainView.projectVerseFromRow`** — `ContentView.swift:1375-1381`
...
- [ ] **`AVerse` not `Identifiable` — index-based SwiftUI diffing** — `Db.swift:4` + `ContentView.swift:505`
